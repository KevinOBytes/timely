...
export async function POST(req: NextRequest) {
  try {
    ...
    const splitPointIso = splitPoint.toISOString();
    
    await db.update(timeEntries).set({
      stoppedAt: splitPoint,
      durationSeconds: newOriginalDuration
    ...
    await appendAuditLog({
      ...
      diff: {
        stoppedAt: { before: previous.stoppedAt, after: splitPointIso },
        durationSeconds: { before: previous.durationSeconds, after: newOriginalDuration },
      },
    });
